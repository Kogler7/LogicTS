        ...
        const onChanged = () => {
            ...
            if (selectedObjects.size === 1) {
                ...
                if (core.isResizable(obj.id)) {
                    ...
                    const boundRect = core
                        .crd2posRect(obj.rect)
                        .padding(cornerSize)
                        .float()
                    ...
                    for (const corner of boundRect.corners) {
                        const cornerRect = new Rect(
                            Point.minus(
                                corner,
                                new Point(halfCorner, halfCorner),
                            ),
            ...
            cacheCtx.beginPath()
            const rects = [...selectedObjects].map((obj) =>
                core.crd2posRect(obj.rect).padding(cornerSize).float(),
            )
            ...
            const boundRect = core
                .crd2posRect(core.selectedLogicBoundRect)
                .padding(cornerSize)
                .float()
