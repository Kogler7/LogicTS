...
export default class RenderNode implements IHashable {
    ...
    public calcRelativePortPos(port: RenderPort, padding = 0): Point {
        ...
        const { loc, asp } = port
        if (asp === PortAspect.LEFT) return new Point(-padding, loc)
        if (asp === PortAspect.RIGHT) return new Point(width + padding, loc)
        if (asp === PortAspect.TOP) return new Point(loc, -padding)
        if (asp === PortAspect.BOTTOM) return new Point(loc, height + padding)
        return Point.zero()
    }
    ...
    public calcPortPos(port: RenderPort, padding = 0): Point {
        return Point.plus(
            this.rect.pos,
            this.calcRelativePortPos(port, padding),
        )
    }
}
